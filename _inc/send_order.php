...
// Include necessary classes and files for handling messages, validation, orders, and rendering popups
...
function wp_atc_register_order(): void
{
    ...
    foreach ($cart_data as $index => $item) {
        ...
        foreach ($product_details['product_materials'] as $item_key => $item_data) {
            ...
            if (!$update_product_material_amount_status) {
                Message::wp_atc_send_json_message('error', 'محصول ' . $product_details['product_name'] . ' با تعداد مورد نظر شما ناموجود است ', 403);
            }
        }
    }
    ...
    // If order creation was successful, store the new order information in a transient
    // if ($order_id) {
    ...
    set_transient('new_orders', $new_orders, 5 * MINUTE_IN_SECONDS);
